...
## Installation
The installation is relatively simple and you can pretty much copy/paste these
commands on either macOS or in a Linux-based environment:
...
```sh
rpk topic create news positive-news negative-news neutral-news unknown-news -p5
```
...
produce and consume from the above topics as well as create consumer groups:
...
```
> Obviously, feel free to use a different password!
...
- **message routing**: how can you sort data into different output topics?
...
Here we use more resource magic to make the outputs toggleable via the
environment variable `RPCN_OUTPUT_MODE`. We start off with a trivial
...
##### HTTP Response
Since this is an HTTP API, it's following what some call a "request/reply"
...
sourcing data from an input Redpanda topic, perforing the same sentiment 
analysis we configured in [python.yaml](./python.yaml), and sort the output
just like before.
...
```sh
rpk topic consume positive-news negative-news neutral-news unknown-news
```
